...
void init_classes(Heap *heap, Module *builtin) {
  ...
  Class_FunctionRef = module_add_class(builtin, FUNCTION_REF_NAME);
  Class_Module = module_add_class(builtin, MODULE_NAME);
  Class_Array = module_add_class(builtin, ARRAY_NAME);
  Class_String = module_add_class(builtin, STRING_NAME);
  Class_Tuple = module_add_class(builtin, TUPLE_NAME);
