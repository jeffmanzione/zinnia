...
POPULATE_IMPL(compound_statement, const SyntaxTree *stree,
  ...
  ExpressionTreeArray_init(&compound_statement->expressions);
  ...
  if (!CHILD_IS_SYNTAX(stree, 1, rule_statement_list)) {
    ...
    *ExpressionTreeArray_push_back_ref(&compound_statement->expressions) = elt;
    return;
  }

  ExpressionTree *first = semantic_analyzer_populate(
      analyzer, CHILD_SYNTAX_AT(CHILD_SYNTAX_AT(stree, 1), 0));
  *ExpressionTreeArray_push_back_ref(&compound_statement->expressions) = first;
  SyntaxTree *cur = CHILD_SYNTAX_AT(CHILD_SYNTAX_AT(stree, 1), 1);
  while (true) {
    if (!IS_SYNTAX(cur, rule_statement_list_inner)) {
      ExpressionTree *elt = semantic_analyzer_populate(analyzer, cur);
      *ExpressionTreeArray_push_back_ref(&compound_statement->expressions) =
          elt;
      break;
    }
    ExpressionTree *elt =
...
DELETE_IMPL(compound_statement, SemanticAnalyzer *analyzer) {
  ...
  ExpressionTreeArray_iterator(&iter, &compound_statement->expressions);
  for (; ExpressionTreeArray_has_next(&iter); ExpressionTreeArray_next(&iter)) {
    ExpressionTree *tree = *ExpressionTreeArray_mutable_value(&iter);
    semantic_analyzer_delete(analyzer, tree);
  }
  ExpressionTreeArray_finalize(&compound_statement->expressions);
}

PRODUCE_IMPL(compound_statement, SemanticAnalyzer *analyzer, Tape *target) {
  int num_ins = 0;
  if (ExpressionTreeArray_size(&compound_statement->expressions) == 0) {
    return 0;
  }
  ExpressionTreeArrayIterator iter;
  ExpressionTreeArray_iterator(&iter, &compound_statement->expressions);
  for (; ExpressionTreeArray_has_next(&iter); ExpressionTreeArray_next(&iter)) {
    const ExpressionTree *tree = *ExpressionTreeArray_value(&iter);
...
POPULATE_IMPL(try_statement, const SyntaxTree *stree,
  ...
  ASSERT(CHILD_IS_TOKEN(stree, 0, KEYWORD_TRY));
