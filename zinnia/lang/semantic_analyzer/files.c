...
void populate_class_def(ClassSignature *def, const SyntaxTree *stree) {
  ClassNameArray_init(&def->parent_classes);
  // No parent classes.
  if (IS_SYNTAX(stree, rule_identifier)) {
    ...
    if (CHILD_IS_SYNTAX(parent_classes, 1, rule_identifier)) {
      ClassName name = {.token = CHILD_SYNTAX_AT(parent_classes, 1)->token,
                        .module = NULL};
      *ClassNameArray_push_back_ref(&def->parent_classes) = name;
...
void populate_field_statements(const SyntaxTree *stree, ClassDef *class) {
  ...
  if (!CHILD_IS_SYNTAX(stree, 1, rule_identifier_list)) {
    ...
    *FieldDefArray_push_back_ref(&class->fields) =
        populate_field_statement(field_token, CHILD_SYNTAX_AT(stree, 1));
    ;
    return;
  }
  *FieldDefArray_push_back_ref(&class->fields) = populate_field_statement(
...
void populate_static_field_statement(SemanticAnalyzer *analyzer,
  ...
  StaticDef *def = StaticDefArray_push_back_ref(&class->statics);
...
void populate_class_statement(SemanticAnalyzer *analyzer, ClassDef *class,
  ...
  if (IS_SYNTAX(stree, rule_field_statement)) {
    ...
    *FunctionDefArray_push_back_ref(&class->methods) =
        populate_method(analyzer, stree);
    ...
    if (class->constructor.has_args) {
      ArgumentArrayIterator args;
      ArgumentArray_iterator(&args, &class->constructor.args.args);
      for (; ArgumentArray_has_next(&args); ArgumentArray_next(&args)) {
        const Argument *arg = ArgumentArray_value(&args);
        if (!arg->is_field) {
          continue;
        }
        FieldDef field = {.name = arg->arg_name,
                          .field_token = arg->field_token};
        *FieldDefArray_push_back_ref(&class->fields) = field;
      }
    }
...
void populate_class_statements(SemanticAnalyzer *analyzer, ClassDef *class,
  ...
  const SyntaxTree *statement = CHILD_SYNTAX_AT(CHILD_SYNTAX_AT(stree, 1), 1);
  while (true) {
    if (!IS_SYNTAX(statement, rule_class_statement_list_inner)) {
...
ClassDef populate_class(SemanticAnalyzer *analyzer, const SyntaxTree *stree) {
  ...
  AnnotationArray_init(&class.annots);
  FieldDefArray_init(&class.fields);
  StaticDefArray_init(&class.statics);
  FunctionDefArray_init(&class.methods);
...
void delete_class(SemanticAnalyzer *analyzer, ClassDef *class) {
  ...
  ClassNameArray_finalize(&class->def.parent_classes);
  FieldDefArray_finalize(&class->fields);

  int i;
  for (i = 0; i < StaticDefArray_size(&class->statics); ++i) {
    StaticDef *sta = StaticDefArray_mutable_ref_unchecked(&class->statics, i);
    semantic_analyzer_delete(analyzer, sta->value);
  }
  StaticDefArray_finalize(&class->statics);

  for (i = 0; i < FunctionDefArray_size(&class->methods); ++i) {
    FunctionDef *func =
        FunctionDefArray_mutable_ref_unchecked(&class->methods, i);
    delete_function(analyzer, func);
  }
  FunctionDefArray_finalize(&class->methods);

  AnnotationArrayIterator annots;
  AnnotationArray_iterator(&annots, &class->annots);
  ...
  AnnotationArray_finalize(&class->annots);
}

int produce_constructor(SemanticAnalyzer *analyzer, const ClassDef *class,
                        Tape *tape) {
  int num_ins = 0;
  ...
  int num_fields = FieldDefArray_size(&class->fields);
  if (num_fields > 0) {
    num_ins += tape_ins_no_arg(tape, PUSH, class->def.name.token);
    int i;
    for (i = 0; i < num_fields; ++i) {
      const FieldDef *field =
          FieldDefArray_get_ref_unchecked(&class->fields, i);
  ...
  if (class->has_constructor) {
    const FunctionDef *func = &class->constructor;
...
int produce_static(SemanticAnalyzer *analyzer, const ClassDef *class,
                   const StaticDef *sta, Tape *tape) {
  int num_ins = 0;
...
int produce_statics(SemanticAnalyzer *analyzer, const ClassDef *class,
                    Tape *tape) {
  int num_ins = 0;
  int num_statics = StaticDefArray_size(&class->statics);
  for (int i = 0; i < num_statics; ++i) {
    const StaticDef *sta = StaticDefArray_get_ref_unchecked(&class->statics, i);
...
int produce_class(SemanticAnalyzer *analyzer, const ClassDef *class,
                  Tape *tape) {
  int num_ins = 0;
  if (ClassNameArray_size(&class->def.parent_classes) > 1) {
    FATALF("Cannot have more than one super class.");
  }
  num_ins += tape_class(tape, class->def.name.token);
  // Fields
  FieldDefArrayIterator fields;
  FieldDefArray_iterator(&fields, &class->fields);
  for (; FieldDefArray_has_next(&fields); FieldDefArray_next(&fields)) {
    const FieldDef *f = FieldDefArray_value(&fields);
    tape_field(tape, f->name->text);
  }
  // Constructor
  if (class->has_constructor || FieldDefArray_size(&class->fields) > 0) {
    num_ins += produce_constructor(analyzer, class, tape);
  }
  int i, num_methods = FunctionDefArray_size(&class->methods);
  for (i = 0; i < num_methods; ++i) {
    const FunctionDef *func =
        FunctionDefArray_get_ref_unchecked(&class->methods, i);
...
void populate_annotation_list(SemanticAnalyzer *analyzer,
  ...
  if (IS_SYNTAX(stree, rule_annotation)) {
    *AnnotationArray_push_back_ref(annots) =
        populate_annotation(analyzer, stree);
    return;
  }
  *AnnotationArray_push_back_ref(annots) =
      populate_annotation(analyzer, CHILD_SYNTAX_AT(stree, 0));
  if (!CHILD_IS_SYNTAX(stree, 1, rule_annotation) &&
      !CHILD_IS_SYNTAX(stree, 1, rule_annotation_list_inner)) {
...
void populate_fi_statement(SemanticAnalyzer *analyzer, const SyntaxTree *stree,
  ...
  if (IS_SYNTAX(stree, rule_module_statement)) {
    ...
    *ImportArray_push_back_ref(&module->imports) =
        populate_import(analyzer, stree);
  } else if (IS_SYNTAX(stree, rule_class_definition_no_annotation)) {
    *ClassDefArray_push_back_ref(&module->classes) =
        populate_class(analyzer, stree);
    ...
    populate_annotation_list(analyzer, CHILD_SYNTAX_AT(stree, 0),
                             &class.annots);
    *ClassDefArray_push_back_ref(&module->classes) = class;
  } else if (IS_SYNTAX(stree, rule_function_definition)) {
    *FunctionDefArray_push_back_ref(&module->functions) =
        populate_function(analyzer, stree);
  } else {
    ExpressionTreeArray_push_back(&module->statements,
                                  semantic_analyzer_populate(analyzer, stree));
  }
}
...
ModuleDef populate_module_def(SemanticAnalyzer *analyzer,
  ...
  ImportArray_init(&module_def.imports);
  ClassDefArray_init(&module_def.classes);
  FunctionDefArray_init(&module_def.functions);
  ExpressionTreeArray_init(&module_def.statements);

  populate_fi_statement(analyzer, CHILD_SYNTAX_AT(stree, 0), &module_def);
  const SyntaxTree *cur = CHILD_SYNTAX_AT(stree, 1);
  while (true) {
    if (!IS_SYNTAX(cur, rule_file_level_statement_list_inner)) {
...
void delete_statement_(SemanticAnalyzer *analyzer, ExpressionTree *statement) {
  semantic_analyzer_delete(analyzer, statement);
}

void delete_module_def(SemanticAnalyzer *analyzer, ModuleDef *module) {
  ImportArray_finalize(&module->imports);

  ClassDefArrayIterator classes;
  for (ClassDefArray_iterator(&classes, &module->classes);
       ClassDefArray_has_next(&classes); ClassDefArray_next(&classes)) {
    delete_class(analyzer, ClassDefArray_mutable_value(&classes));
  }
  ClassDefArray_finalize(&module->classes);

  FunctionDefArrayIterator functions;
  for (FunctionDefArray_iterator(&functions, &module->functions);
       FunctionDefArray_has_next(&functions);
       FunctionDefArray_next(&functions)) {
    delete_function(analyzer, FunctionDefArray_mutable_value(&functions));
  }
  FunctionDefArray_finalize(&module->functions);

  ExpressionTreeArrayIterator statements;
  for (ExpressionTreeArray_iterator(&statements, &module->statements);
       ExpressionTreeArray_has_next(&statements);
       ExpressionTreeArray_next(&statements)) {
    delete_statement_(analyzer,
                      *ExpressionTreeArray_mutable_value(&statements));
  }
  ExpressionTreeArray_finalize(&module->statements);
}
...
int produce_function_annotation(SemanticAnalyzer *analyzer,
  ...
  num_ins += tape_ins_text(tape, CALL, global_intern("add_annotation"),
                           annot->class_name);
  num_ins += tape_ins_no_arg(tape, RES, annot->class_name);
...
int produce_module_def(SemanticAnalyzer *analyzer, ModuleDef *module,
  ...
  int i;
  // Imports
  for (i = 0; i < ImportArray_size(&module->imports); ++i) {
    const Import *import = ImportArray_get_ref_unchecked(&module->imports, i);
  ...
  // Superclasses
  for (i = 0; i < ClassDefArray_size(&module->classes); ++i) {
    const ClassDef *class =
        ClassDefArray_get_ref_unchecked(&module->classes, i);
    if (ClassNameArray_size(&class->def.parent_classes) == 0) {
      continue;
    }
    const ClassName *super =
        ClassNameArray_get_ref_unchecked(&class->def.parent_classes, 0);
    num_ins += tape_ins(tape, PUSH, class->def.name.token);
    ...
    num_ins +=
        tape_ins_text(tape, CALL, global_intern("$__set_super"), super->token);
  }
  // Function annotations
  for (i = 0; i < FunctionDefArray_size(&module->functions); ++i) {
    const FunctionDef *func =
        FunctionDefArray_get_ref_unchecked(&module->functions, i);
    AnnotationArrayIterator annots;
    AnnotationArray_iterator(&annots, &func->annots);
    for (; AnnotationArray_has_next(&annots); AnnotationArray_next(&annots)) {
      num_ins += produce_function_annotation(
          analyzer, func, AnnotationArray_value(&annots), tape);
    }
  }
  // Class annotations
  for (i = 0; i < ClassDefArray_size(&module->classes); ++i) {
    const ClassDef *class =
        ClassDefArray_get_ref_unchecked(&module->classes, i);
    for (int j = 0; j < FunctionDefArray_size(&class->methods); ++j) {
      const FunctionDef *func =
          FunctionDefArray_get_ref_unchecked(&class->methods, j);
      AnnotationArrayIterator annots;
      AnnotationArray_iterator(&annots, &func->annots);
      for (; AnnotationArray_has_next(&annots); AnnotationArray_next(&annots)) {
        num_ins += produce_method_annotation(
            analyzer, class, func, AnnotationArray_value(&annots), tape);
      }
    }
    ...
    AnnotationArray_iterator(&annots, &class->annots);
    for (; AnnotationArray_has_next(&annots); AnnotationArray_next(&annots)) {
      num_ins += produce_class_annotation(analyzer, class,
                                          AnnotationArray_value(&annots), tape);
    }
  }
  // Statics
  for (i = 0; i < ClassDefArray_size(&module->classes); ++i) {
  ...
  // Statements
  for (i = 0; i < ExpressionTreeArray_size(&module->statements); ++i) {
    ExpressionTree *statement =
        ExpressionTreeArray_get_unchecked(&module->statements, i);
