...
DEFINE_ARRAYLIKE(TapePtrArray, Tape *);
IMPL_ARRAYLIKE(TapePtrArray, Tape *);

IMPL_ARRAYLIKE(NamedArgumentArray, NamedArgument);
IMPL_ARRAYLIKE(PostfixArray, Postfix);
IMPL_ARRAYLIKE(ConditionalArray, Conditional);
IMPL_ARRAYLIKE(ArgumentArray, Argument);
IMPL_ARRAYLIKE(AnnotationArray, Annotation);
IMPL_ARRAYLIKE(MapDecEntryArray, MapDecEntry);
IMPL_ARRAYLIKE(AssignmentArray, Assignment);
IMPL_ARRAYLIKE(ClassNameArray, ClassName);
IMPL_ARRAYLIKE(FieldDefArray, FieldDef);
IMPL_ARRAYLIKE(StaticDefArray, StaticDef);
IMPL_ARRAYLIKE(FunctionDefArray, FunctionDef);
IMPL_ARRAYLIKE(ImportArray, Import);
IMPL_ARRAYLIKE(ClassDefArray, ClassDef);
IMPL_ARRAYLIKE(BiSuffixArray, BiSuffix);
...
POPULATE_IMPL(tuple_expression, const SyntaxTree *stree,
              SemanticAnalyzer *analyzer) {
  ExpressionTreeArray_init(&tuple_expression->list);
  APPEND_TREE(analyzer, &tuple_expression->list, CHILD_SYNTAX_AT(stree, 0));
  DECLARE_IF_TYPE(tuple1, rule_tuple_expression_inner,
  ...
  while (true) {
    ...
    ASSIGN_IF_TYPE(tuple1, rule_tuple_expression_inner,
...
DELETE_IMPL(tuple_expression, SemanticAnalyzer *analyzer) {
  int i;
  for (i = 0; i < ExpressionTreeArray_size(&tuple_expression->list); ++i) {
    semantic_analyzer_delete(analyzer, ExpressionTreeArray_get_unchecked(
                                           &tuple_expression->list, i));
  }
  ExpressionTreeArray_finalize(&tuple_expression->list);
}
...
int tuple_expression_helper(SemanticAnalyzer *analyzer,
                            ...
                            Tape *tape) {
  int i, num_ins = 0,
         tuple_len = ExpressionTreeArray_size(&tuple_expression->list);
...
PRODUCE_IMPL(tuple_expression, SemanticAnalyzer *analyzer, Tape *target) {
  ...
  num_ins += tape_ins_int(target, TUPL,
                          ExpressionTreeArray_size(&tuple_expression->list),
...
PRODUCE_IMPL(array_declaration, SemanticAnalyzer *analyzer, Tape *target) {
  ...
  if (IS_EXPRESSION(array_declaration->exp, tuple_expression)) {
    ...
    num_members = ExpressionTreeArray_size(&tuple_expression->list);
...
void populate_named_argument_(SemanticAnalyzer *analyzer,
...
POPULATE_IMPL(named_argument_list, const SyntaxTree *stree,
              SemanticAnalyzer *analyzer) {
  NamedArgumentArray_init(&named_argument_list->list);
  populate_named_argument_(
      analyzer, CHILD_SYNTAX_AT(stree, 0),
      NamedArgumentArray_push_back_ref(&named_argument_list->list));
  DECLARE_IF_TYPE(tuple1, rule_named_argument_list_inner,
                  CHILD_SYNTAX_AT(stree, 1));
  // First comma.
  ...
  while (true) {
    ASSERT(CHILD_IS_TOKEN(tuple1, 0, SYMBOL_COMMA));
    populate_named_argument_(
        analyzer, CHILD_SYNTAX_AT(tuple1, 1),
        NamedArgumentArray_push_back_ref(&named_argument_list->list));
    if (CHILD_COUNT(tuple1) == 2) {
      break;
    }
    ASSERT(CHILD_COUNT(tuple1) == 3);
    ASSIGN_IF_TYPE(tuple1, rule_named_argument_list_inner,
                   CHILD_SYNTAX_AT(tuple1, 2));
  }
}
...
PRODUCE_IMPL(named_argument_list, SemanticAnalyzer *analyzer, Tape *target) {
  ...
  NamedArgumentArrayIterator iter;
  NamedArgumentArray_iterator(&iter, &named_argument_list->list);
  for (; NamedArgumentArray_has_next(&iter); NamedArgumentArray_next(&iter)) {
    const NamedArgument *arg = NamedArgumentArray_value(&iter);
...
DELETE_IMPL(named_argument_list, SemanticAnalyzer *analyzer) {
  ...
  NamedArgumentArray_iterator(&iter, &named_argument_list->list);
  for (; NamedArgumentArray_has_next(&iter); NamedArgumentArray_next(&iter)) {
    NamedArgument *arg = NamedArgumentArray_mutable_value(&iter);
    semantic_analyzer_delete(analyzer, arg->value);
  }
  NamedArgumentArray_finalize(&named_argument_list->list);
}

POPULATE_IMPL(named_argument, const SyntaxTree *stree,
              SemanticAnalyzer *analyzer) {
  populate_named_argument_(analyzer, stree, &named_argument->arg);
}
...
void postfix_period(const SyntaxTree *suffix, PostfixArray *suffixes) {
  Postfix *postfix = PostfixArray_push_back_ref(suffixes);
  postfix->type = Postfix_field;
  postfix->token = CHILD_SYNTAX_AT(suffix, 0)->token;
  postfix->id = NULL;
  postfix->exp = NULL;
...
void postfix_surround_helper(SemanticAnalyzer *analyzer,
                             const SyntaxTree *suffix, PostfixArray *suffixes,
  ...
  Postfix postfix = {.type = postfix_type,
                     ...
                     .id = NULL,
                     .exp = NULL};
  ASSERT(!HAS_TOKEN(suffix));
  ASSERT(CHILD_IS_TOKEN(suffix, 0, opener));
  if (CHILD_IS_TOKEN(suffix, 1, closer)) {
    // No args.
    postfix.exp = NULL;
    *PostfixArray_push_back_ref(suffixes) = postfix;
    return;
  }
...
POPULATE_IMPL(postfix_expression, const SyntaxTree *stree,
  ...
  PostfixArray_init(&postfix_expression->suffixes);
...
int produce_postfix(SemanticAnalyzer *analyzer, int *i, int num_postfix,
                    ...
                    Tape *tape) {
  int num_ins = 0;
  const Postfix *cur = *next;
  *next = (*i + 1 == num_postfix)
              ? NULL
              : PostfixArray_get_ref_unchecked(suffixes, *i + 1);
...
PRODUCE_IMPL(postfix_expression, SemanticAnalyzer *analyzer, Tape *target) {
  int i, num_ins = 0,
         num_postfix = PostfixArray_size(&postfix_expression->suffixes);
  num_ins +=
      semantic_analyzer_produce(analyzer, postfix_expression->prefix, target);
  const Postfix *next =
      PostfixArray_get_ref_unchecked(&postfix_expression->suffixes, 0);
...
DELETE_IMPL(postfix_expression, SemanticAnalyzer *analyzer) {
  ...
  int i;
  for (i = 0; i < PostfixArray_size(&postfix_expression->suffixes); ++i) {
    Postfix *postfix =
        PostfixArray_mutable_ref_unchecked(&postfix_expression->suffixes, i);
    if (postfix->type != Postfix_field && NULL != postfix->exp) {
      semantic_analyzer_delete(analyzer, postfix->exp);
    }
  }
  PostfixArray_finalize(&postfix_expression->suffixes);
}
...
PRODUCE_IMPL(range_expression, SemanticAnalyzer *analyzer, Tape *target) {
  int num_ins = 0;
  num_ins += tape_ins_text(target, PUSH, global_intern("range"),
...
#define POPULATE_BI_EXPRESSION_IMPL(expr, stree, analyzer)               \
  {                                                                      \
    expr->exp =                                                          \
        semantic_analyzer_populate(analyzer, CHILD_SYNTAX_AT(stree, 0)); \
    BiSuffixArray_init(&expr->suffixes);                                 \
    ...
    while (true) {                                                       \
      ...
      if (second_exp->rule_fn == stree->rule_fn) {                       \
        ...
        *BiSuffixArray_push_back_ref(&expr->suffixes) = suffix;          \
...
#define DELETE_BI_EXPRESSION_IMPL(expr, analyzer)                      \
  {                                                                    \
    ...
    BiSuffixArrayIterator iter;                                        \
    BiSuffixArray_iterator(&iter, &expr->suffixes);                    \
    for (; BiSuffixArray_has_next(&iter); BiSuffixArray_next(&iter)) { \
      BiSuffix *suffix = BiSuffixArray_mutable_value(&iter);           \
      semantic_analyzer_delete(analyzer, suffix->exp);                 \
    }                                                                  \
    BiSuffixArray_finalize(&expr->suffixes);                           \
  }
...
PRODUCE_IMPL(and_expression, SemanticAnalyzer *analyzer, Tape *target) {
  TapePtrArray and_bodies;
  TapePtrArray_init(&and_bodies);
  int num_suffixes = BiSuffixArray_size(&and_expression->suffixes);
  int num_ins =
      semantic_analyzer_produce(analyzer, and_expression->exp, target);
  int i, and_suffix_ins = 0;
  for (i = 0; i < num_suffixes; ++i) {
    const BiSuffix *suffix =
        BiSuffixArray_get_ref_unchecked(&and_expression->suffixes, i);
    Tape *and_tape = tape_create();
    and_suffix_ins +=
        semantic_analyzer_produce(analyzer, suffix->exp, and_tape);
    TapePtrArray_push_back(&and_bodies, and_tape);
  }

  for (i = 0; i < num_suffixes; ++i) {
    const BiSuffix *suffix =
        BiSuffixArray_get_ref_unchecked(&and_expression->suffixes, i);
    Tape *and_tape = TapePtrArray_get_unchecked(&and_bodies, i);
  ...
  TapePtrArray_finalize(&and_bodies);
  return num_ins;
}
...
PRODUCE_IMPL(or_expression, SemanticAnalyzer *analyzer, Tape *target) {
  TapePtrArray or_bodies;
  TapePtrArray_init(&or_bodies);
  int num_suffixes = BiSuffixArray_size(&or_expression->suffixes);
  int num_ins = semantic_analyzer_produce(analyzer, or_expression->exp, target);
  int i, or_suffix_ins = 0;
  for (i = 0; i < num_suffixes; ++i) {
    const BiSuffix *suffix =
        BiSuffixArray_get_ref_unchecked(&or_expression->suffixes, i);
    Tape *or_tape = tape_create();
    or_suffix_ins += semantic_analyzer_produce(analyzer, suffix->exp, or_tape);
    TapePtrArray_push_back(&or_bodies, or_tape);
  }

  for (i = 0; i < num_suffixes; ++i) {
    const BiSuffix *suffix =
        BiSuffixArray_get_ref_unchecked(&or_expression->suffixes, i);
    Tape *or_tape = TapePtrArray_get_unchecked(&or_bodies, i);
  ...
  TapePtrArray_finalize(&or_bodies);
  return num_ins;
}
...
void populate_if_else(SemanticAnalyzer *analyzer, IfElse *if_else,
  ...
  ConditionalArray_init(&if_else->conditions);
  ...
  while (true) {
    ...
    *ConditionalArray_push_back_ref(&if_else->conditions) = cond;
...
void delete_if_else(SemanticAnalyzer *analyzer, IfElse *if_else) {
  int i;
  for (i = 0; i < ConditionalArray_size(&if_else->conditions); ++i) {
    Conditional *cond =
        ConditionalArray_mutable_ref_unchecked(&if_else->conditions, i);
    semantic_analyzer_delete(analyzer, cond->condition);
    semantic_analyzer_delete(analyzer, cond->body);
  }
  ConditionalArray_finalize(&if_else->conditions);
...
int produce_if_else(SemanticAnalyzer *analyzer, IfElse *if_else, Tape *tape) {
  int i, num_ins = 0, num_conds = ConditionalArray_size(&if_else->conditions),
         num_cond_ins = 0, num_body_ins = 0;

  TapePtrArray conds;
  TapePtrArray_init(&conds);
  TapePtrArray bodies;
  TapePtrArray_init(&bodies);
  for (i = 0; i < num_conds; ++i) {
    const Conditional *cond =
        ConditionalArray_get_ref_unchecked(&if_else->conditions, i);
    ...
    TapePtrArray_push_back(&conds, condition);
    TapePtrArray_push_back(&bodies, body);
  }
  ...
  for (i = 0; i < ConditionalArray_size(&if_else->conditions); ++i) {
    const Conditional *cond =
        ConditionalArray_get_ref_unchecked(&if_else->conditions, i);
    Tape *condition = TapePtrArray_get_unchecked(&conds, i);
    Tape *body = TapePtrArray_get_unchecked(&bodies, i);
  ...
  TapePtrArray_finalize(&conds);
  TapePtrArray_finalize(&bodies);
  return num_ins;
}
...
void add_arg(Arguments *args, Argument *arg) {
  *ArgumentArray_push_back_ref(&args->args) = *arg;
  if (arg->has_default) {
    args->count_optional++;
  } else {
    args->count_required++;
...
void populate_function_qualifier_(const SyntaxTree *fn_qualifier,
...
void populate_function_qualifiers(const SyntaxTree *fn_qualifiers,
  ...
  if (IS_SYNTAX(fn_qualifiers, rule_function_qualifier)) {
    populate_function_qualifier_(fn_qualifiers, is_const, const_token, is_async,
                                 async_token);
    ...
    populate_function_qualifier_(CHILD_SYNTAX_AT(fn_qualifiers, 0), is_const,
                                 const_token, is_async, async_token);
    populate_function_qualifier_(CHILD_SYNTAX_AT(fn_qualifiers, 1), is_const,
                                 const_token, is_async, async_token);
  } else {
...
void delete_argument_elt_(SemanticAnalyzer *analyzer, Argument *arg) {
  delete_argument(analyzer, arg);
}

void delete_arguments(SemanticAnalyzer *analyzer, Arguments *args) {
  ArgumentArrayIterator iter;
  ArgumentArray_iterator(&iter, &args->args);
  for (; ArgumentArray_has_next(&iter); ArgumentArray_next(&iter)) {
    delete_argument_elt_(analyzer, ArgumentArray_mutable_value(&iter));
  }
  ArgumentArray_finalize(&args->args);
}
...
void delete_function(SemanticAnalyzer *analyzer, FunctionDef *func) {
  ...
  AnnotationArrayIterator annots;
  AnnotationArray_iterator(&annots, &func->annots);
  for (; AnnotationArray_has_next(&annots); AnnotationArray_next(&annots)) {
    delete_annotation(analyzer, AnnotationArray_mutable_value(&annots));
  }
  AnnotationArray_finalize(&func->annots);
  semantic_analyzer_delete(analyzer, func->body);
}

int produce_argument(const Argument *arg, Tape *tape) {
  if (arg->is_field) {
...
int produce_all_arguments(SemanticAnalyzer *analyzer, const Arguments *args,
                          Tape *tape) {
  int i, num_ins = 0, num_args = ArgumentArray_size(&args->args);
  for (i = 0; i < num_args; ++i) {
    const Argument *arg = ArgumentArray_get_ref_unchecked(&args->args, i);
    if (arg->has_default) {
      if (args->is_named) {
...
int produce_arguments(SemanticAnalyzer *analyzer, const Arguments *args,
  ...
  int num_args = ArgumentArray_size(&args->args);
  int i, num_ins = 0;
  if (num_args == 1) {
    const Argument *arg = ArgumentArray_get_ref_unchecked(&args->args, 0);
    if (arg->has_default) {
  ...
  const Argument *first = ArgumentArray_get_ref_unchecked(&args->args, 0);
...
int produce_function_name(const FunctionDef *func, Tape *tape) {
...
FunctionDef populate_anon_function(SemanticAnalyzer *analyzer,
                                   const SyntaxTree *stree) {
  FunctionDef func;
  AnnotationArray_init(&func.annots);
...
POPULATE_IMPL(map_declaration, const SyntaxTree *stree,
  ...
  MapDecEntryArray_init(&map_declaration->entries);
  // Only 1 entry.
  if (IS_SYNTAX(body, rule_map_declaration_entry)) {
    MapDecEntry entry = populate_map_dec_entry(analyzer, body);
    *MapDecEntryArray_push_back_ref(&map_declaration->entries) = entry;
    return;
  }
  // Multiple entries.
  ...
  *MapDecEntryArray_push_back_ref(&map_declaration->entries) = first;
...
DELETE_IMPL(map_declaration, SemanticAnalyzer *analyzer) {
  ...
  int i;
  for (i = 0; i < MapDecEntryArray_size(&map_declaration->entries); ++i) {
    MapDecEntry *entry =
        MapDecEntryArray_mutable_ref_unchecked(&map_declaration->entries, i);
    semantic_analyzer_delete(analyzer, entry->lhs);
    semantic_analyzer_delete(analyzer, entry->rhs);
  }
  MapDecEntryArray_finalize(&map_declaration->entries);
}

PRODUCE_IMPL(map_declaration, SemanticAnalyzer *analyzer, Tape *target) {
  int num_ins = 0, i;
  num_ins += tape_ins_text(target, PUSH, global_intern("struct"),
                           map_declaration->lbrce);
  num_ins +=
      tape_ins_text(target, CLLN, global_intern("Map"), map_declaration->lbrce);
  if (map_declaration->is_empty) {
    return num_ins;
  }
  num_ins += tape_ins_no_arg(target, PUSH, map_declaration->lbrce);
  for (i = 0; i < MapDecEntryArray_size(&map_declaration->entries); ++i) {
    const MapDecEntry *entry =
        MapDecEntryArray_get_ref_unchecked(&map_declaration->entries, i);
...
void populate_single_complex(SemanticAnalyzer *analyzer,
  ...
  PostfixArray_init(&single->suffixes);
  single->prefix_exp =
      semantic_analyzer_populate(analyzer, CHILD_SYNTAX_AT(stree, 0));
  ...
  while (true) {
    ...
    if (IS_SYNTAX(cur, rule_field_expression1) ||
      ...
      *PostfixArray_push_back_ref(&single->suffixes) = postfix;
...
MultiAssignment populate_list(SemanticAnalyzer *analyzer,
  ...
  assignment.subargs = AssignmentArray_create();
  ASSERT(CHILD_IS_TOKEN(stree, 0, open));
  ASSERT(CHILD_IS_TOKEN(stree, 2, close));
  SyntaxTree *list = CHILD_SYNTAX_AT(stree, 1);

  if (HAS_TOKEN(list) ||
      !CHILD_IS_SYNTAX(list, 1, rule_assignment_tuple_list_inner)) {
    // Single element list.
    Assignment subarg = populate_assignment(analyzer, list);
    *AssignmentArray_push_back_ref(assignment.subargs) = subarg;
  } else {
...
void delete_assignment(SemanticAnalyzer *analyzer, Assignment *assignment) {
  if (assignment->type == Assignment_single) {
    ...
    if (single->type == SingleAssignment_complex) {
      ...
      PostfixArrayIterator iter;
      PostfixArray_iterator(&iter, &single->suffixes);
      for (; PostfixArray_has_next(&iter); PostfixArray_next(&iter)) {
        delete_postfix(analyzer, PostfixArray_mutable_value(&iter));
      }
      PostfixArray_finalize(&single->suffixes);
    } else {
      // Is SingleAssignment_var, so do nothing.
    }
  } else {
    AssignmentArrayIterator iter;
    AssignmentArray_iterator(&iter, assignment->multi.subargs);
    for (; AssignmentArray_has_next(&iter); AssignmentArray_next(&iter)) {
      delete_assignment(analyzer, AssignmentArray_mutable_value(&iter));
    }
    AssignmentArray_delete(assignment->multi.subargs);
  }
}
...
int produce_assignment_multi(SemanticAnalyzer *analyzer,
                             const MultiAssignment *multi, Tape *tape,
                             const Token *eq_token) {
  int i, num_ins = 0, len = AssignmentArray_size(multi->subargs);

  num_ins += tape_ins_no_arg(tape, PUSH, eq_token);
  for (i = 0; i < len; ++i) {
    const Assignment *assign =
        AssignmentArray_get_ref_unchecked(multi->subargs, i);
...
int produce_assignment(SemanticAnalyzer *analyzer, const Assignment *assign,
  ...
  if (assign->type == Assignment_single) {
    const SingleAssignment *single = &assign->single;
    if (single->type == SingleAssignment_var) {
      ...
      ASSERT(single->prefix_exp != NULL);
      num_ins += tape_ins_no_arg(tape, PUSH, eq_token);
      num_ins += semantic_analyzer_produce(analyzer, single->prefix_exp, tape);
      int i, len = PostfixArray_size(&single->suffixes);
      const Postfix *next =
          PostfixArray_get_ref_unchecked(&single->suffixes, 0);
      for (i = 0; i < len - 1; ++i) {
        if (NULL == next) {
          break;
        }
      ...
      const Postfix *postfix =
          PostfixArray_get_ref_unchecked(&single->suffixes, len - 1);
