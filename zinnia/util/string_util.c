...
int escape(const char str[], char **target) {
  ...
  while ('\0' != (c = *ptr)) {
    ...
    escaped_str[escaped_len++] = escape_char_(c);
    ptr++;
  }
...
int unescape(const char str[], char **target) {
  ...
  while ('\0' != (c = *ptr)) {
    ...
    if (c == '\\') {
      ptr++;
      unescaped_str[unescaped_len++] = unescape_char_(*ptr);
    } else {
