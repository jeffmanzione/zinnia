...
void optimizer_ResPush(OptimizeHelper *oh, const Tape *const tape, int start,
  ...
  for (i = start + 1; i < end; i++) {
    ...
    if ((RES == first->op || IRES == first->op) &&
        ...
        NULL == IntIntMap_find_ref(&oh->i_gotos, i - 1, sizeof(int))) {
...
void optimizer_PushRes(OptimizeHelper *oh, const Tape *const tape, int start,
  ...
  for (i = start + 1; i < end; i++) {
    ...
    if (PUSH != first->op || RES != second->op || first->type != second->type ||
        NULL != IntIntMap_find_ref(&oh->i_gotos, i, sizeof(int))) {
      continue;
    }
...
void optimizer_Increment(OptimizeHelper *oh, const Tape *const tape, int start,
  ...
  for (i = start + 3; i < end; i++) {
    ...
    if (PUSH == first->op && INSTRUCTION_ID == first->type &&
        ...
        NULL == IntIntMap_find_ref(&oh->i_gotos, i, sizeof(int)) &&
        NULL == IntIntMap_find_ref(&oh->i_gotos, i - 1, sizeof(int)) &&
        NULL == IntIntMap_find_ref(&oh->i_gotos, i - 2, sizeof(int)) &&
        NULL == IntIntMap_find_ref(&oh->i_gotos, i - 3, sizeof(int))) {
...
void optimizer_Nil(OptimizeHelper *oh, const Tape *const tape, int start,
  ...
  for (i = start; i < end; i++) {
    ...
    o_Replace(oh, i, for_op_(insc->op == RES ? RNIL : PNIL));
  }
}
...
void optimizer_StringConcat(OptimizeHelper *oh, const Tape *const tape,
  ...
  for (i = start + 2; i < end; i++) {
    ...
    if (PUSH == first->op && INSTRUCTION_STRING == first->type &&
        ...
        NULL == IntIntMap_find_ref(&oh->i_gotos, i - 1, sizeof(int))) {
      CharPtrArray strs;
      CharPtrArray_init(&strs);
      CharPtrArray_push_back(&strs, (char *)first->str);
      CharPtrArray_push_back(&strs, (char *)second->str);
      ...
      for (j = i + 3; j < end; j += 3) {
        ...
        if (PUSH == push1->op && INSTRUCTION_NO_ARG == push1->type &&
            ...
            NULL == IntIntMap_find_ref(&oh->i_gotos, j - 2, sizeof(int)) &&
            NULL == IntIntMap_find_ref(&oh->i_gotos, j - 1, sizeof(int))) {
          *CharPtrArray_push_back_ref(&strs) = (char *)push2->str;
          o_Remove(oh, j - 2);
          o_Remove(oh, j - 1);
          o_Remove(oh, j);
        } else {
      ...
      CharPtrArray_iterator(&iter, &strs);
      for (; CharPtrArray_has_next(&iter); CharPtrArray_next(&iter)) {
        const char *str = *CharPtrArray_value(&iter);
      ...
      tmp_str[total_len] = '\0';
      const char *new_str = global_intern(tmp_str);
      Instruction new_ins = {
