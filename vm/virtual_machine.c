        ...
        if (NULL == task->parent_task) {
          ...
          for (; has(&dependent_tasks); inc(&dependent_tasks)) {
            ...
            *task_mutable_resval(dependent_task) = *task_get_resval(task);
            process_enqueue_task(process, dependent_task);
            process_remove_waiting_task(process, dependent_task);
          }
        }
        break;
      case TASK_COMPLETE:
...
ThreadHandle process_run_in_new_thread(Process *process) {
  return thread_create(AS_VOID_FN(_process_run_return_void_ptr), process);
}